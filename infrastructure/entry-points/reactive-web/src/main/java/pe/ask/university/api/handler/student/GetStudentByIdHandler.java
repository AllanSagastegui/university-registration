...
/**
 * Handles the HTTP GET request for retrieving a student by their ID.
 * <p>
 * This handler is responsible for processing requests to fetch a single student
 * based on their unique identifier. It extracts the ID from the request path,
...
public class GetStudentByIdHandler {
    ...
    private final IGetStudentByIdUseCase useCase;
    private final IStudentMapper mapper;

    /**
     * Listens for and processes the GET request to retrieve a student by their ID.
     * <p>
     * This method orchestrates the flow for fetching a student:
     * <ol>
     *     <li>Extracts the student ID from the request's path variable.</li>
     *     <li>Invokes the {@link IGetStudentByIdUseCase} to get the student.</li>
     ...
     */
    public Mono<ServerResponse> listenGETStudentByIdUseCase(ServerRequest serverRequest) {
        UUID id = UUID.fromString(serverRequest.pathVariable("id"));
        return useCase.getStudentById(id)
